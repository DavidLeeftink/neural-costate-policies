...
from NeuralCostatePolicies.rl.policy.policy import CTRNNActorCritic
...
def main():
    ...
    # Save to a dedicated RNN folder so standard PPO logs are safe
    script_dir = os.path.dirname(os.path.abspath(__file__))
    save_dir = os.path.join(script_dir, args.task, "combined_logs_rnn")    
    os.makedirs(save_dir, exist_ok=True)
    
    # Give this specific seed a unique metrics file name
    metrics_path = os.path.join(save_dir, f"metrics_seed_{args.seed}.json")

    print(f"Running Model: NC-CTRNN PPO | Task: {args.task} | Seed: {args.seed}")
    ...
    recurrent_policy = CTRNNActorCritic(
        key=subkey, 
    ...
    print(f"✅ Saved NC-CTRNN model and stats for seed {args.seed}")
