...
def get_config(mjx_config, env_name, D_sys, D_obs, D_control, seed):
...
def main():
    ...
    save_dir = os.path.join(script_dir, args.task, "combined_logs_rnn")    
    os.makedirs(save_dir, exist_ok=True)
    
    metrics_path = os.path.join(save_dir, f"metrics_seed_{args.seed}_with_initnetwork.json")

    print(f"Running Model: NC-GRU | Task: {args.task} | Seed: {args.seed}")
    ...
    env_config = get_config(mjx_env_config, env_name, args.D_sys, args.D_obs, args.D_control, args.seed)
    ...
    model_path = os.path.join(save_dir, f"policy_seed_{args.seed}_with_initnetwork.eqx")
    stats_path = os.path.join(save_dir, f"obs_stats_seed_{args.seed}_with_initnetwork.eqx")
    
    eqx.tree_serialise_leaves(model_path, trained_rnn_ppo_policy)
    eqx.tree_serialise_leaves(stats_path, rnn_ppo_stats)
    print(f"✅ Saved NC-GRU model and stats for seed {args.seed}")
